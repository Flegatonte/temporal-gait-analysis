...
class CasiaBDataset(Dataset):
    def __init__(
        ...
        """
        casia-b silhouette dataset loader.

        expected layout:
            <data_root>/<subject_id>/<seq_type>/<view>/*.png

        returns:
            seq:  tensor [1, T, H, W] in [0, 1]
            label: int (mapped from subject_id within the selected split)
            meta:  dict with seq_path, label_orig, type, view

        eval_mode (test only):
            - "center_crop": take a fixed-length central clip (fast, default)
            - "full_seq":    load the full sequence (used for chunked evaluation)
        """
        ...
        # build a stable label mapping within the split
        ...
        for p in self.seq_paths:
            ...
            self.labels.append(self.id2label[sid])
    ...
    def augment_sequence(self, seq_imgs):
        ...
        # random erasing (p=0.5), applied per-frame
        if random.random() < 0.5:
            ...
            for i in range(len(seq_imgs)):
                ...
                if eh > 0 and ew > 0 and eh < h and ew < w:
    ...
    def load_sequence(self, seq_path):
        ...
        # if a sequence folder is empty, return a zero clip to avoid crashing downstream
        if num_frames == 0:
        ...
        # sampling strategy:
        # - train: random contiguous clip of length seq_len (or repeat-pad if too short)
        # - test:
        #   - center_crop: fixed central clip of length seq_len (or repeat-pad)
        #   - full_seq: load all frames (optionally subsample to limit memory)
        if self.split == "train":
            ...
            if self.eval_mode == "full_seq":
                ...
                # safety: reduce memory if sequences are extremely long
                if len(indices) > 500:
                    indices = indices[::2]
            else:
                if num_frames < self.seq_len:
        ...
        # load frames as grayscale, resize to (resolution, resolution), normalize to [0, 1]
        seq_imgs = []
        for i in indices:
        ...
        seq_tensor = torch.from_numpy(seq_np).unsqueeze(0)  # [1, T, H, W]
